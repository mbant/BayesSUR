...
plotMCMCdiag <- function(object, nbloc=4){
  ...
  dens.last <- density(logP[Ptau.indx, -c(1:floor(ncol(logP)/2))])#  ,main="",col="black",add=TRUE)
  ...
  list.dens <- NULL
  for (i in 1:nbloc){
    dens <- density(logP[Ptau.indx, floor((ncol(logP))/2)+(1+mid*(i-1)):(mid*nbloc)])
  ...
  plot(logP[Plik.indx,], xlab="Iterations (*1000)", ylab="Log likelihood (posterior)", type="l", lty=1)
  #legend("topleft", legend=paste("Chain",1:nChain), lty=1, cex=0.5)
  
  plot(model_size, xlab="Iterations (*1000)", ylab="Model size", type="l", lty=1)
  #legend("topleft", legend=paste("Chain",1:nChain), lty=1, cex=0.5)
  ...
  legend("topleft",title="sweep",legend=paste(c("ALL","First half","Last half")," = [",c(1,1,(ncol(logP))/2*1000+1),":",c(ncol(logP),floor((ncol(logP))/2),ncol(logP))*1000,"]",sep=""),col=1:3,lty=1,text.col=1:3, cex=0.5)
  
  for (i in 1:nbloc){
    plot(list.dens[[i]],col=i,xlim=c(xmin2,xmax2),ylim=c(ymin,ymax2),xlab="",main=title.0)
    par(new=TRUE)  
  }
  legend("topleft",title="moving window",legend=paste("set ",1:nbloc," = [",(floor((ncol(logP))/2)+mid*(nbloc:1-1))*1000+1,":",(ncol(logP))*1000,"]",sep=""),col=1:nbloc,lty=1,text.col=1:nbloc, cex=0.5)
