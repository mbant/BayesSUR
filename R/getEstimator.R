...
#' @param estimator the name of one estimator. Default is the latent indicator estimator "gamma". Other options "beta" and "Gy" correspond the posterior means of coefficient matrix and response graph, respectively 
#' @export
getEstimator <- function(object, estimator="gamma"){
  
  devAskNewPage(FALSE)
  object$output[-1] <- paste(object$output$outFilePath,object$output[-1],sep="")
  if( sum(estimator %in% c("gamma","beta","Gy"))<1 )
